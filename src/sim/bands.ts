 ...
 *    (실측). 그래서 못의 값은 **한 번 재서 못 박는다**.
 *
 *    ★ 왜 못 박는가 — 재계산하면 반드시 발산한다.
 *      못은 조합 앙상블에서 재고, 그 앙상블의 부착물이 곧 튜닝 대상이다. 아이템을
 *      밴드에 맞추려 올리면 못이 오르고, 못이 오르면 밴드가 올라 다시 올려야 한다.
 *      패스마다 재게 했을 때 171% → 392% → 234% → 53,713% → 5,707,701% →
 *      523,851,963% 로 폭발했고, 세션당 한 번으로 줄여도 세션을 넘기며
 *      171% → 400% 로 드리프트했다. 목표가 대상의 함수인 한 고정점은 없다.
 *
 *    ★ 왜 171.2 인가 — v3 재밸런싱 **시작 시점의 카탈로그**에서 잰 값이다.
 *      (맨총에서 재면 33.9% 인데, 아이템 값은 부착물이 달린 조합에서 재므로 단위가
 *      어긋난다 — 그 못으로 판정하면 커먼 열 장의 중앙값 약 230% 가 전부 '초과' 로
 *      찍혀 카탈로그를 7배 깎아야 한다는 잘못된 결론이 나온다.)
 *      이 수는 보정 상수다. 다시 재는 것은 되지만 그건 **의도적인 설계 결정**이어야
 *      하고, 그때 카탈로그 전체가 같이 움직인다는 것을 알고 해야 한다.
 *
 *  탄: **일반 특수탄 한 발 = 기본탄 4발**.
 ...
 * 자동 재계산은 발산한다. 못을 의도적으로 옮길 때만 쓴다.
 */
