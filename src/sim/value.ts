 ...
 *   이 지표에서 몇 %인지는 게임의 산수(온도 누적)가 정한다.
 *
 *   ★ **아이템을 재는 것과 같은 조합 앙상블**에서 잰다. 맨총에서 재면 33.9% 가
 *   나오는데, 아이템 값은 부착물이 달린 조합에서 재므로 단위가 어긋난다 —
 *   그 못으로 판정하면 커먼 열 장의 중앙값(약 230%)이 전부 '초과' 로 찍혀
 *   카탈로그를 7배 깎아야 한다는 잘못된 결론이 나온다.
 *
 *   ★★ 대신 **세션당 한 번만** 잰다. 패스마다 다시 재면 목표가 튜닝 대상에
 *   의존하는 양의 되먹임이 된다 — 아이템을 밴드에 맞추려 올리면 못이 오르고,
 *   못이 오르면 밴드가 올라 다시 올려야 한다. 실측 발산이 정확히 그랬다:
 *   패스마다 171% → 392% → 234% → 53,713% → 5,707,701% → 523,851,963%.
 */
